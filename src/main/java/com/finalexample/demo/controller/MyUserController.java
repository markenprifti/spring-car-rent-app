...
public class MyUserController extends BaseController{
    ...
    @PostMapping("/reserve")
    public String reserveCarForUser(@ModelAttribute("addReservationRequest")AddReservationRequest addReservationRequest,
                                    Model model) {
        ...
        return error("This car is reserved on these dates." +
                " Please provide a different range.", model);

    }

    @GetMapping("/reserve")
    public String myReservationView(@PageableDefault(value = 0, size = 5) Pageable pageable, Model model){

        String username = extractUsername();
        MyReservationsResponse myReservationsResponse = reservationService.getReservationsByUsername(username, pageable);
